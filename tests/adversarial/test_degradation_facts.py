...
def test_accumulation_of_benign_denials_does_not_escalate_past_cautious():
    ...
    clean = TaintState()
    for _ in range(10):
        engine.record_signal(_ni(), _deny(), clean)
...
def test_single_untrusted_dangerous_deny_restricts_immediately():
    ...
    engine = DegradationEngine(DegradationPolicy())
    tainted = TaintState(sources=frozenset({TaintSource.WEB}))
    t = engine.record_signal(_ni(tool="bash", operation="execute"), _deny(), tainted)
...
def test_tool_pressure_counter_kept_as_telemetry_not_driver():
    ...
    engine = DegradationEngine(DegradationPolicy())
    tainted = TaintState(sources=frozenset({TaintSource.WEB}))
    engine.record_signal(_ni(tool="bash", operation="execute"), _deny(), tainted)
