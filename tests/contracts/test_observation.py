...
def test_view_carries_probe_expected_fields() -> None:
    # The exact attribute set axor-probe's _SessionContextViewLike declares —
    # renaming any of these is a cross-repo breaking change (P-34).
    v = to_session_context_view(_ctx(), session_id="s", agent_id="", timestamp=0.0, system_prompt_hash="")
    for attr in (
        ...
        assert hasattr(v, attr), attr


def test_emit_accepts_a_list_of_taps_and_isolates_failures() -> None:
    # GovernedSession's public parameter is context_taps (a list); one failing
    # tap must not starve the others.
    seen: list[SessionContextView] = []
    ...
    asyncio.run(emit_context_view([_Bad(), _Good()], _ctx(), session_id="s1"))
    assert len(seen) == 1
    asyncio.run(emit_context_view([], _ctx(), session_id="s1"))  # empty → noop
