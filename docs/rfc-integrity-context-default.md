...
Status: **draft** · Scope: integrity axis of the per-value taint gate · Confidentiality: unchanged
...
| Federation L2 restore | `federation/gateway.py` when a receipt restores a `constant()` root | Only a verified receipt can make a foreign value trusted, the same condition under which it may lower trust today. |
...
- **Inter-node messages** (`axor_core/node/messaging.py`). A received value is
  model-generated by the sender. It joins the receiver's context root with the
  folded root it carries. It is trusted only if it arrived with a clean root
  **and** the sender's context was clean. That condition is added to the envelope
  as `sender_context_root`.
...
`TrustedOrigin` is a small enum: `TASK`, `OPERATOR`, `TOOL`, `ENDORSED`, `PEER`.
...
2. **Engine + contract** behind `integrity_default="context"` (off by default).
   ...
   `f(x) ∈ Trusted`.
3. **Boundaries**: spawn, messaging, federation, endorsement (§3.4).
...
1. Should Library/Production keep heuristic-clean reads trusted (parity), or
   require `benign_tools` in `context` mode as Strict does?
2. Free-text trusted index: are identifier tokens plus whole lines enough, or do
   adapters need a structured `register_trusted` per extracted field?
3. Should `context_root` be per node or per session for in-process children with
   federation off? This RFC takes per node, inherited at spawn.
4. Do messages need `sender_context_root` on the wire, or is re-minting every
   received value as context-tainted (simpler, coarser) acceptable?
