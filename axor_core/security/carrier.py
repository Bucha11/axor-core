"""Deterministic carrier classifier.

`classify_carrier` projects a value onto the imperative-channel lattice
ENDORSED ⊏ CLOSED_SCHEMA ⊏ FREE_TEXT. It is deterministic and structural — it
reads the *form* of the value, never a model, never the value's "meaning". A
model-based classifier would let the projection be steered by the governed
content; this one cannot.

Rules (fail-closed — anything ambiguous is the top of the lattice, FREE_TEXT):
...
Note: a number is instruction-free *as a carrier*, but it can still become
instruction-complete *by use* if a consumer decodes it into an opcode/index. The
carrier check only removes the imperative channel of the *form*; the data-flow
channel is tracked separately by causal_root.
"""
...
# interpreter treats as syntax. Conservative on purpose. `/` and `:` are
# excluded: they would turn a path or a URL into a bounded identifier, which
# would let `/etc/passwd`, `http://attacker/...` or `host:port` masquerade as
# CLOSED_SCHEMA and slip through the positional admission gate for operator-
# declared instruction-incomplete sinks. A path/URL is FREE_TEXT.
...
def _classify_leaf(value: object) -> Carrier:
    ...
    if isinstance(value, float):
        # A non-finite float (inf / nan) is an anomalous form, so fail closed —
...
def classify_carrier(value: object) -> Carrier:
    """Deterministic, structural carrier classification. Total function;
    fail-closed to FREE_TEXT for anything not provably lower-capacity.
    """
    if value is None or isinstance(value, (int, float, bool)):
        return _classify_leaf(value)  # finite check for inf/nan floats
    ...
    if isinstance(value, str):
        ...
        # and nan evades numeric_range checks downstream.
        if s and s[0] in "{[":
            try:
